...
# In this part one we want to find with bag can contains a specifig type+color bag
...
It's the same here, we can buid a network to follow the "contains" path this time, and explore the path starting fom `shiny_gold` nodes, but we are interesting in the `edges` of the path, that inform us about the number of the bags. Let's do this in the test case above.
...
```
We got the correct awnser, so we repiclate this to the input data.
